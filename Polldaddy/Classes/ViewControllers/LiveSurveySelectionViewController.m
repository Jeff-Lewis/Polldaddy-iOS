        ...
        if ( [url compare:[NSString stringWithString:[Configuration sharedInstance].polldaddyUrl]] == NSOrderedSame ) {
            ...
            if ( res ) {
                if ( res.content.count == 1 )
                    ...
                    [cellController setStatus:[NSString stringWithFormat:@"Retrieving resource %u of %lu (%@)", ( res.current + 1 ), (unsigned long)( res.content.count + 1 ), [NSString stringPrettyBytes:bytes]] forCell:cell];
            }
        }
